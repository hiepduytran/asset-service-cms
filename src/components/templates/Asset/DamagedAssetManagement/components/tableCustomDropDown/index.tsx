...
type Props = {
  ...
  tabName: string
  handleFetchDataChild?: (id: number) => void
  objectShowDropdown?: {
    header: string
    fieldName: string
  }
...
export const TableWithDropdownDamagedAssetManagement = ({
  className,
  tableName,
  tableNameChild,
  data,
  ...
  const handleOpen = (index: number) => {
    setOpenStates((prevOpenStates) => {
      ...
      if (!prevOpenStates[index]) {
        handleFetchDataChild && handleFetchDataChild(data[index].id)
  ...
  const [columnsZero, ...restColumns] = columns

  useEffect(() => {
    setOpenStates([])
  }, [data.length])

  const dataColumn = isShowDropdown
    ? isShowColumnStt
      ? [
          columnsZero,
          {
  ...
  if (isShowDropdown && objectShowDropdown) {
