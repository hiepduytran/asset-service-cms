...
export const useTableStep3 = (props: { index: number; isView: boolean }) => {
  const { t } = useTranslation(TRANSLATE.WEEKLY_MAINTENANCE_PLAN)
  const { index, isView } = props
      ...
      [
        ...
        },
        {
          header: (
            <>
              {t('table.jobContent')}
              {!isView && <span className='text-[#FF4956]'> *</span>}
            </>
          ),
          fieldName: 'content',
        },
  ...
  const dataTableStep3 = (
    watch(`planConfig.${index}.planConfigMaintenance`) ?? []
