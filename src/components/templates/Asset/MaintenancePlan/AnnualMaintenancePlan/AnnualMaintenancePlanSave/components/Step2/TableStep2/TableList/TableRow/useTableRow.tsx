...
export const useTableRow = ({ index, index1, isView }: {
  index: number, index1: number, isView: boolean
}) => {
  ...
  const tableData = fieldsConfigLine.map((item: any, index2: number) => {
    return {
      ...
      product: <AutoCompleteAPIStep2
        ...
        params={{ accessoryId: watch(`planConfig.${index}.planConfigMaintenance.${index1}.accessoryId`) }}
        onChangeValue={(data) => {
        ...
        disabled={!(watch(`planConfig.${index}.planConfigMaintenance.${index1}.maintenanceAccessory`))}
      />,
      uom: <CoreInput
        control={control}
