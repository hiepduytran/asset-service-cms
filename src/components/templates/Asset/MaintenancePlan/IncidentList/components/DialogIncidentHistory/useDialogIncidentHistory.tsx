...
import { DialogIncidentDetail } from '../DialogIncidentDetail'
...
export const useDialogIncidentHistory = (props: any) => {
    ...
    const tableData = (data?.data?.content ?? []).slice().reverse().map((item: any, index: number) => {
        ...
        return {
                ...
                onClick={() => {
                    showDialog2(
                        <DialogIncidentDetail
                            ...
                            location={item?.incidentLocation?.name}
            ...
            >
                <Typography color={BLUE}>{t('common:btn.viewDetail')}</Typography>
            </IconButton>)
        }
    })
